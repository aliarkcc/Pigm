...
namespace Mvc.Controllers
{
    public class IhracatController : Controller
    {
        ...
        public async Task<IActionResult> Index(string currentFilter, string searching, int? page)
        {
            ...
            var followList = await _httpClient.GetFromJsonAsync<ApiResponseModel<FollowList>>(url + "FollowLists/GetList");
        ...
        public async Task<IActionResult> Add(FollowList followList)
        {
            ...
            HttpResponseMessage responseMessage = await _httpClient.PostAsJsonAsync(url + "FollowLists/GetList",followList);
        ...
        {
            var result = _followListService.GetById(id);
            if (result.Success)
            {
        ...
        [HttpPost]
        public IActionResult Update(FollowList followList)
        {
            followList.ImportExportId = 1;
            followList.CompanyId = 1;
            _followListService.Update(followList);
            return RedirectToAction("Index", "Ihracat");
        }
        [HttpGet]
        public IActionResult Delete(int id)
        {
            var result = _followListService.Delete(id);
            if (result.Success)
            {
                return RedirectToAction("Index", "Ihracat");
            }
